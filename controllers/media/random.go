...
func (mc *Controller) GetRandom(c *fiber.Ctx) error {
	...
	mediaObjects, err := mc.storage.GetRandom(ctx, 2, "track")
	if err != nil {
	...
	mc.storage.Log.Info().Msgf("Got %d random media items", len(mediaObjects))
	mediaItems := make([]media.Details, len(mediaObjects))

	errChan := make(chan mediaError, len(mediaObjects))
	...
	for id, kind := range mediaObjects {
		wg.Add(1)
